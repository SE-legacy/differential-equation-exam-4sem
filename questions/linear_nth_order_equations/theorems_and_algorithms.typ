...
*Следствие*: пусть $phi_1 (x), ..., phi_m (x)$ --- произвольное решение $y^((n)) + a_1 (x) y^((n-1)) + ... + a_n (x) y = 0$, $alpha_1, ..., alpha_m$ --- произвольные числа.
...
$ y(x) = c_1 phi_1(x) + c_2 phi_2(x) + ... + c_n phi_n(x), $ <general_solution>
...
$==>$ по теореме из алгебры системы @linear_system_of_solutions имеет единственное решение $c_1^0, ..., c_n^0$.
...
$limits(==>)_"единственность решений" u(x) equiv z(x)$, $u(x)$ и $z(x)$ являются решениями одной и той же задачи Коши.
...
Обозначим
$ u(x) = z(x) - y_1(x). $ <equation_neodnorod_4>

Рассмотрим $l(u(x)) = underbrace(l(z(x)), equiv f(x)) - underbrace(l(y_1 (x)), equiv f(x)) equiv 0$
...
$phi_1 (x), phi_2 (x)$ --- фундаментальные системы решений.
